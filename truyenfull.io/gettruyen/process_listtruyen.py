...
TRUYENIO = "truyenfull.io"
TRUYENTV = "truyenfull.tv"
...
def crawl_story(story_name, total_chapters):
    try:
        url = f"https://{TRUYENIO}/{story_name}/"
        ...
        if thong_tin_truyen:
            ...
            query = """
                ...
                (title, title_full, author, status, chapter_number, image, description, "views", rating, updated_at, external_id)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), nextval('app_truyen_story_external_id_seq'::regclass));
            """
        ...
        with open(output_file, "w", encoding="utf-8") as file:
            ...
            if missing_chapters:
                ...
                for chapter in missing_chapters:
                    link = f"https://{TRUYENIO}/{story_name}/chuong-{chapter}"
                    file.write(link + "\n")
        ...
        with open("listtruyen.txt", "r", encoding="utf-8") as infile:
            for line in infile:
                try:
                    ...
                    if not check_story_exists(story_name, cursor):
...
def process_truyen(root_dir, sql_file):
    for subdir in os.listdir(root_dir):
        ...
        if os.path.isdir(subdir_path):
            ...
            truyenfull_path = os.path.join(subdir_path, TRUYENIO)
